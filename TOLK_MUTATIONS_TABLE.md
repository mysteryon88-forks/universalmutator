...
- для `universalmutator/comby/tolk.rules` — Comby; этот слой теперь зеркалит большую часть Tolk-specific surface-мутаций без отдельных Comby-only Tolk-правил, а некоторые line-oriented вещи вроде single-line field default removal всё ещё остаются regex-only.
...
| Есть (служебное) | комментарии `/*`                                                                                                                                                                                                            | `/\* ==> SKIP_MUTATING_REST`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | прекращает мутации на оставшейся части комментария                                                                                                   | низкий        |
...
| Есть             | default parameter value                                                                                                                                                                                                     | regex: `(\bfun ...)\s*=\s*... ==> ...` для single-line сигнатур;<br>Comby: `fun :[name](:[before], :[arg]: :[type] = :[default], :[after]) ==> fun :[name](:[before], :[arg]: :[type], :[after])`                                                                                                                                                                                                                                                                                                                                                                                                       | убирает documented default parameter value в сигнатуре функции                                                                                       | средний       |
| Есть             | default field value в single-line struct declaration                                                                                                                                                                        | `(^\s*[A-Za-z_]\w*\s*:\s*[A-Za-z_][A-Za-z0-9_<>\?\|\(\),\s]*?)\s*=\s*[^,\n]+(\s*(?://[^\n]*)?$) ==> \1\2`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | убирает documented default initializer у поля, сохраняя тип и комментарий                                                                            | средний       |
...
| Есть             | send mode constants                                                                                                                                                                                                         | `\bSEND_MODE_REGULAR\b ==> SEND_MODE_CARRY_ALL_REMAINING_MESSAGE_VALUE`<br>`\bSEND_MODE_CARRY_ALL_REMAINING_MESSAGE_VALUE\b ==> SEND_MODE_REGULAR`<br>`\bSEND_MODE_CARRY_ALL_REMAINING_MESSAGE_VALUE\b ==> SEND_MODE_CARRY_ALL_BALANCE`<br>`\bSEND_MODE_CARRY_ALL_BALANCE\b ==> SEND_MODE_REGULAR`<br>`\bSEND_MODE_CARRY_ALL_BALANCE\b ==> SEND_MODE_CARRY_ALL_REMAINING_MESSAGE_VALUE`<br>`\bSEND_MODE_IGNORE_ERRORS\b ==> 0`<br>`\bSEND_MODE_PAY_FEES_SEPARATELY\b ==> 0`<br>`\bSEND_MODE_BOUNCE_ON_ACTION_FAIL\b ==> 0`<br>`\bSEND_MODE_DESTROY\b ==> 0`<br>`\bSEND_MODE_ESTIMATE_FEE_ONLY\b ==> 0` | подменяет documented base modes и удаляет send mode flags/constants, включая destroy и estimate-fee-only                                             | очень высокий |
...
| Есть             | numeric mode literals в `.send(...)`, `.sendAndEstimateFee(...)`, `.estimateFeeWithoutSending(...)`, `sendRawMessage(..., mode)`, `reserveToncoinsOnBalance(..., mode)` и `reserveExtraCurrenciesOnBalance(..., ..., mode)` | для message helper-ов: `<code>0 -&gt; 64</code>`, `<code>1/2/16/32/1024 -&gt; 0</code>`, `<code>64 -&gt; 0 / 128</code>`, `<code>128 -&gt; 0 / 64</code>`;<br>для reserve helper-ов: `<code>0 -&gt; 1 / 2</code>`, `<code>1 / 2 -&gt; 0</code>`                                                                                                                                                                                                                                                                                                                                                        | мутирует те же message/reserve режимы, когда код использует literal-числа вместо stdlib-констант, включая fee-estimation и extra-currencies helper-ы | очень высокий |
...
- `@inline`, `@noinline`, `@inline_ref`, `@pure` — это документированные атрибуты функций и методов;
...
- они меняют inlining и purity assumptions;
- это хороший класс мутаций именно для Tolk, а не общий операторный шум.

Удаление default parameter value сейчас есть в обоих слоях:

- regex покрывает однострочные сигнатуры и остаётся самым простым способом держать эту мутацию рядом с остальными Tolk surface-rules;
- Comby дублирует ту же идею структурнее и обычно устойчивее к вариациям пробелов и формы аргументов.
...
- те же numeric-мутации для literal-режимов в `.send(...)`, `.sendAndEstimateFee(...)`, `.estimateFeeWithoutSending(...)`, `sendRawMessage(..., mode)`, `reserveToncoinsOnBalance(..., mode)` и `reserveExtraCurrenciesOnBalance(..., ..., mode)`, включая `0 -> 64` для message helper-ов и `0 -> 1 / 2` для reserve helper-ов;
...
Отдельных Comby-only string-мутаций в Tolk-слое больше не оставлено:

- legacy `stringCrc*` / `stringSha256*` синхронизированы между `static` и `comby`;
- более хрупкая перестановка двух соседних `StringBuilder.append(...)` в цепочке убрана из `comby/tolk.rules`, чтобы не держать отдельную Comby-only ветку.
