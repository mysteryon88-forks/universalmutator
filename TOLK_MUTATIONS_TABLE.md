...
- для `universalmutator/comby/tolk.rules` — Comby; этот слой теперь зеркалит большую часть Tolk-specific surface-мутаций, а line-level replacements вроде broad `throw 0xFFFF` остаются regex-only.
