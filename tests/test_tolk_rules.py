...
class TestTolkRules(TestCase):
    def test_tolk_specific_regex_rules_generate_expected_mutants(self):
        ...
        self.assertIn("fun helper(mutate cs: slice, mutate amount: int, target: cell?, body: slice) {", mutant_lines)
        self.assertIn("    if (amount > 0) throw ERR;", mutant_lines)
        self.assertIn("    var parsed = target;", mutant_lines)
        self.assertIn("    var casted = parsed;", mutant_lines)
        ...
        self.assertIn("    var lt = blockchain.now();", mutant_lines)
        ...
        self.assertIn("    var offchain = metadataUri.prefixWith01();", mutant_lines)
        self.assertIn("    var regular = metadataUri.prefixWith00();", mutant_lines)
        self.assertIn("    var offchain = metadataUri;", mutant_lines)
        self.assertIn("    var regular = metadataUri;", mutant_lines)
        self.assertIn('    val crc = "some_str".crc16();', mutant_lines)
        self.assertIn('    val crcSmall = "some_str".crc32();', mutant_lines)
        self.assertIn('    val hash = "some_crypto_key".sha256_32();', mutant_lines)
        self.assertIn('    val minihash = "some_crypto_key".sha256();', mutant_lines)
